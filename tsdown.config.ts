 ...
 *
 * Emits two artifacts the same way the harness's in-repo plugins do:
 *  - lib/index.js  — the Host half (plain node ESM library)
 *  - lib/client.js — the browser client bundle, a closure-factory artifact
 *    calling `window.__ModuleLoader__.load` with the plugin id, resolving
...
export default defineConfig([
  // Host half: plain node ESM library.
  {
    name: ID,
    ...
    platform: 'node',
    target: 'es2024',
    // Keep the .js extension so package.json main/exports resolve it.
    fixedExtension: false,
    dts: false,
    clean: false,
  },
  // Client half: browser bundle in the __ModuleLoader__ closure shape.
  {
    name: `${ID}/client`,
